...
export default function JobInput() {
  ...
  return (
    <div>
      ...
      <textarea id="job-description" name="job-description" value={rawText} disabled={isLoading} onChange={(event) => { setRawText(event.target.value); setData(null); setWarnings([]); setError(""); }} aria-describedby={`jd-help${error ? " jd-error" : ""}`} className="mt-5 block min-h-72 w-full resize-y rounded-xl border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-700 outline-none transition-colors placeholder:text-slate-400 focus:border-blue-500 focus:ring-3 focus:ring-blue-100 disabled:bg-slate-50" placeholder={"请粘贴目标岗位的完整 JD，例如：\n\n岗位职责：\n1. 负责经营数据分析……\n2. 制作业务分析报告……\n\n任职要求：\n1. 熟练使用 Excel……\n2. 具备良好的数据分析能力……"} />
      <p className="mt-3 text-xs leading-5 text-slate-500">开发测试：点击后将 JD 发送到当前 AI 服务（默认 DeepSeek），仅提取岗位要求，不进行简历匹配或优化。</p>
      <button type="button" disabled={isLoading} onClick={parseJob} className="mt-3 w-full rounded-lg border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-medium text-blue-700 hover:bg-blue-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600 disabled:cursor-wait disabled:opacity-60">{isLoading ? "AI 正在识别岗位要求…" : "AI 解析岗位 JD"}</button>
